...
겉으로 드러난 오류보다, 그 원인을 만든 흐름을 분석하며 해결합니다.<br>
단순히 폰트 용량이 아니라 번들 순서에 따른 요청 우선순위와 렌더링 단계의 병목을 추적해 페이지 로딩 지연을 해결했습니다.

모든 문제 해결 과정은 기록으로 남기고, 그 기록을 다음 설계의 기준으로 삼습니다.<br>
트러블슈팅 중 시도와 실패를 GitHub Issue와 Notion에 정리해, 이후 기능 개발 시 동일 오류를 방지하는 내부 표준 문서로 발전시켰습니다.
